...
#define PARSE_CONFIG_STR(kw, arg, val, maxlen) \
        if( strncmp( arg, #kw, strlen(#kw) ) == 0 ){ \
            strncpy( kw, val, maxlen-1); \
            continue; \
        }
    ...
    if ( config_file ){
        ...
        if( fp ){
            ...
            while( fgets( line, sizeof(line), fp ) != NULL ){
                ...
                PARSE_CONFIG_STR(modelfile, keyword, value, 63);
                PARSE_CONFIG_STR(mode     , keyword, value, 7);
                PARSE_CONFIG_INT(port     , keyword, value);
                PARSE_CONFIG_STR(whitelist, keyword, value, 255);
...
#define OPTARG_READ_INT(optarg, arg) \
        if( strncmp( arg, "--" #optarg, strlen("--" #optarg) ) == 0 ){ \
            char *endptr = NULL;                                             \
            optarg = strtol( strchr(arg, '=' ) + 1, &endptr, 10 ); \
            break; \
        }
#define OPTARG_READ_STRING(optarg, arg, maxlen) \
        if( strncmp( arg, "--" #optarg, strlen("--" #optarg) ) == 0 ){ \
            strncpy( optarg, strchr(arg, '=' ) + 1, maxlen ); \
            break; \
        }

    for ( int i = 1 ; i < argc ; i++ ){
        OPTARG_READ_INT(port, argv[i]);
        OPTARG_READ_STRING(modelfile, argv[i], 63);
        OPTARG_READ_STRING(mode,      argv[i], 7);
        OPTARG_READ_STRING(whitelist, argv[i], 255);
